...
export type ScrollbackRenderContext = {
  width: number
...
export function entryWriter(input: {
  ...
  return (ctx) => {
    const body = entryBody(input.commit)
    const structured =
      body.type !== "structured"
        ? undefined
        : body.snapshot.kind === "diff"
          ? "Applied diff"
          : body.snapshot.kind === "question"
            ? "Questions"
            : body.snapshot.kind === "todo"
              ? "Todos"
              : body.snapshot.title
    const content =
      body.type === "none"
        ...
        : body.type === "structured"
          ? structured
          : body.content
    const style = entryLook(input.commit, (input.theme ?? RUN_THEME_FALLBACK).entry)
    return {
      root: new TextRenderable(ctx.renderContext, {
        id: "run-scrollback-entry",
