...
import { entryWriter, sameEntryGroup, separatorRows, spacerWriter, type ScrollbackWriter } from "./scrollback.writer"
...
type ScrollbackSurface = {
  renderContext: ConstructorParameters<typeof TextRenderable>[0]
  root: { add: (renderable: unknown) => void }
  height: number
  isDestroyed: boolean
  render: () => void
  settle: () => Promise<void>
  commitRows: (start: number, end: number, options: { trailingNewline: boolean }) => void
  destroy: () => void
}

type ScrollbackRenderer = CliRenderer & {
  createScrollbackSurface?: (options: { startOnNewLine?: boolean }) => ScrollbackSurface
  writeToScrollback?: (writer: ScrollbackWriter) => void
}
...
export class RunScrollbackStream {
  ...
  private scrollback: ScrollbackRenderer
  ...
  constructor(
    ...
    this.scrollback = this.renderer as ScrollbackRenderer
  ...
  private createEntry(commit: StreamCommit, body: ActiveBody): ActiveEntry {
    const surface = this.scrollback.createScrollbackSurface?.({
      startOnNewLine: entryFlags(commit).startOnNewLine,
    })
    if (!surface) {
      throw new Error("Scrollback surface unavailable")
    }
  ...
  private writeSpacer(rows: number): void {
    ...
    this.scrollback.writeToScrollback?.(spacerWriter())
    this.wrote = false
  }
  ...
  private async flushActive(done: boolean, trailingNewline: boolean): Promise<boolean> {
    ...
    renderable.streaming = !done
    await active.surface.settle()
    const targetBlockCount = done
      ? renderable._blockStates.length
      : Math.max(active.committedBlocks, renderable._blockStates.length - 1)
  ...
  public async append(commit: StreamCommit): Promise<void> {
    if (!this.scrollback.writeToScrollback || !this.scrollback.createScrollbackSurface) {
      return
    }
    ...
    this.scrollback.writeToScrollback(
      entryWriter({
        commit,
        theme: this.theme,
        opts: {
